...
| `COMP-3` packed decimal | In scope, **reached and mapped** | In scope | Corrected 2026-08-07 by an exhaustive numeric-field cross-check (`tests/unit/test_numeric_field_coverage.py`). The earlier verdict here — "not present in Track C's scope" — was derived by reading every copybook the four programs `COPY`, where it is still true that none declares `COMP-3`. It does not hold for the programs' own source: `CBACT01C` declares `OUT-ACCT-CURR-CYC-DEBIT` and `ARR-ACCT-CURR-CYC-DEBIT` as `PIC S9(10)V99 USAGE IS COMP-3` in its `FILE SECTION`. `pic_mapper` always detected `COMP-3` correctly; nothing could reach it until ADR-0011. `OUT-ACCT-CURR-CYC-DEBIT` now maps (`BigDecimal`, precision 12, scale 2, signed); `ARR-ACCT-CURR-CYC-DEBIT` is isolated instead, for the unrelated reason in the `OCCURS` row below. Note for Milestone C4 ([ADR-0035](adr/0035-fixed-occurs-stays-unrepresentable-and-cbact01c-demo-outputs-stay-out-of-generate.md)): both live in `CBACT01C`'s scoped-out demo outputs, so **no `COMP-3` field is on the generation path** — the two business programs' copybooks declare none. Parsed and mapped is not the same as generated, and this row is about the former. |
