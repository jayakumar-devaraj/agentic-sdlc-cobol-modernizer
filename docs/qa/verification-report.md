...
| 09 | [The write path, the systemic gate fixes, and the round trips](verification/09-the-write-path-and-the-round-trips.md) | Splitting logic from wiring, `1300-B-WRITE-TX` as its own step, the systemic halves of G26/G27/G28, the oracle's first record, and the round trips run with and without a model writing the bodies. |
