...
| 02 | [Spec extraction, critique, and Milestone C2's numeric-field gate](verification/02-spec-extraction-critique-and-the-c2-numeric-gate.md) | `spec_extractor` and `spec_critic` against real Track C source, `source_units`, the CBACT04C golden fixture, `gate_items`, the generated schemas, Milestone C2's numeric-field gate, and the untrusted boundary asserted against the prompts the nodes actually send — including the billed run confirming the critic still discriminates with the narration inside it. |
