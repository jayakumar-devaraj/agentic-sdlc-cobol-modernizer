 ...
 * one that does not.
 *
 * Reused verbatim from agentic-sdlc-control-plane, where "the diagrams render on
 * GitHub" broke three times - twice while diagrams were being written, once from a
 * search-and-replace that inserted backticks into a node label. Every time, the
 * repository looked fine locally and GitHub showed "Unable to render rich display"
 * on the rendered page - a failure visible only to someone browsing the repo, which
 * is exactly the audience these diagrams exist for.
 *
